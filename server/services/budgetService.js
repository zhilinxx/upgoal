...
export async function buildDashboardData(userId) {
    ...
    // --- Check "Other" spending ratio ---
    const otherThisMonth = await getOtherSpendThisMonth(userId);
    const otherRatio = netIncome > 0 ? otherThisMonth / Number(netIncome) : 0;
    const otherGt10 = otherRatio > 0.10;
    console.log("[Other Spend]", { otherThisMonth, otherRatio, otherGt10 });
    ...
    try {
        const { data } = await axios.post(
            process.env.AI_BUDGET_URL || "http://localhost:5002/api/segment",
            {
                ...
                overspend_flags: { other_gt_10: otherGt10 },
            },
            { timeout: 2000 }
        );
